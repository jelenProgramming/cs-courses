...
export const treesContent = {
  'bst': {
    ...
    how: [
      ...
      { en: 'Insert: search for the key; the empty slot where the search ends is where the new node goes.', de: 'Einfuegen: nach dem Schluessel suchen; die leere Stelle, an der die Suche endet, ist der Platz des neuen Knotens.' },
      { en: 'Delete: leaf - just remove it; one child - splice it out; two children - replace with the in-order successor.', de: 'Loeschen: Blatt - einfach entfernen; ein Kind - heraustrennen; zwei Kinder - durch In-Order-Nachfolger ersetzen.' },
    ],
    paper: [
      { en: 'Draw the root, then for each value: start at the root and write the comparison (&lt; goes left, &gt; goes right) until you reach an empty spot.', de: 'Wurzel zeichnen, dann fuer jeden Wert: an der Wurzel beginnen und den Vergleich notieren (&lt; nach links, &gt; nach rechts), bis ein freier Platz erreicht ist.' },
      { en: '<strong>In-order traversal</strong> (left, node, right) prints a BST in sorted order - the #1 way to check your tree is correct.', de: '<strong>In-Order-Durchlauf</strong> (links, Knoten, rechts) gibt einen BST sortiert aus - der beste Weg, die Korrektheit zu pruefen.' },
...
Node* search(Node* root, int k) {
    ...
    complexity: [
      { case: { en: 'Search/Insert/Delete - balanced', de: 'Suchen/Einf./Loeschen - balanciert' }, time: 'O(log n)', space: 'O(n)', cls: 'best' },
      { case: { en: 'Search/Insert/Delete - average', de: 'Suchen/Einf./Loeschen - Durchschnitt' }, time: 'O(log n)', space: 'O(n)', cls: 'avg' },
  ...
  'trie': {
    ...
    how: [
      ...
      { en: 'Insert: follow/create one edge per character; mark the final node as a word end.', de: 'Einfuegen: pro Zeichen eine Kante folgen/erstellen; den letzten Knoten als Wortende markieren.' },
  ...
  'linked-list': {
    ...
    how: [
      ...
      { en: 'Insert at head: point the new node at the old head, move head. No shifting, unlike an array.', de: 'Am Kopf einfuegen: neuen Knoten auf alten Kopf zeigen, Kopf verschieben. Kein Verschieben wie beim Array.' },
    ],
    paper: [
      ...
      { en: 'To delete a node: redraw the neighbor arrows to skip it. For a doubly linked list update BOTH directions.', de: 'Zum Loeschen: die Nachbarpfeile neu zeichnen, sodass der Knoten uebersprungen wird. Bei doppelter Liste BEIDE Richtungen.' },
    ],
...
void insertAtHead(Element*& head, Element*& tail, int value) {
    ...
    complexity: [
      ...
      { case: { en: 'Insert / Delete at head or tail', de: 'Einf./Loeschen an Kopf/Schwanz' }, time: 'O(1)', space: 'O(n)', cls: 'best' },
      { case: { en: 'Insert / Delete at known node', de: 'Einf./Loeschen an bekanntem Knoten' }, time: 'O(1)', space: 'O(n)', cls: 'best', note: { en: 'Compare with an array: array access is O(1) but middle insert is O(n); a list is the reverse.', de: 'Vergleich mit Array: Array-Zugriff O(1), aber Einfuegen in der Mitte O(n); Liste umgekehrt.' } },
    ],
    pitfalls: [
      ...
      { en: 'Always update <strong>both</strong> prev and next when deleting from a doubly linked list, or you corrupt it.', de: 'Beim Loeschen aus einer doppelt verketteten Liste <strong>beide</strong> Zeiger (prev und next) aktualisieren, sonst beschaedigt.' },
    ],
  },
  'stack-queue': {
    ...
    how: [
      ...
      { en: 'Queue: enqueue adds to the rear, dequeue removes from the front.', de: 'Warteschlange: enqueue hinten anfuegen, dequeue vorne entnehmen.' },
    ...
    complexity: [
      ...
      { case: { en: 'Search', de: 'Suche' }, time: 'O(n)', space: 'O(n)', cls: 'worst', note: { en: 'Both give O(1) for their defined operations. Searching inside them is not what they are for.', de: 'Beide bieten O(1) fuer ihre Operationen. Suche darin ist nicht ihr Zweck.' } },
    ],
    pitfalls: [
      { en: 'Stack = LIFO, Queue = FIFO. Mixing them up is the most basic exam slip.', de: 'Stapel = LIFO, Warteschlange = FIFO. Verwechslung ist der einfachste Pruefungsfehler.' },
      { en: 'Circular queue: distinguishing "full" from "empty" needs care (leave one slot or keep a count).', de: 'Ringpuffer: „voll" von „leer" zu unterscheiden braucht Sorgfalt (einen Platz frei lassen oder zaehlen).' },
    ],
  },
  'hash-table': {
    tldr: { en: 'Stores key-value pairs by running the key through a hash function to pick a bucket. <strong>O(1) average</strong> for insert/lookup/delete, but <strong>O(n) worst case</strong> when everything collides into one bucket. The average is why hash tables are everywhere.', de: 'Speichert Schluessel-Wert-Paare, indem der Schluessel durch eine Hashfunktion einen Behaelter waehlt. <strong>O(1) im Durchschnitt</strong> fuer Einfuegen/Suchen/Loeschen, aber <strong>O(n) im schlechtesten Fall</strong>, wenn alles in einen Behaelter kollidiert.' },
    when: { en: 'The default for fast key-value lookup: dictionaries, sets, caches, database indexes, deduplication. Use unless you need sorted order (then use a balanced tree).', de: 'Der Standard fuer schnelles Schlussel-Wert-Nachschlagen: Woerterbuecher, Mengen, Caches, DB-Indizes. Nutze es, ausser du brauchst sortierte Ordnung (dann balancierter Baum).' },
    how: [
      { en: 'A hash function maps each key to a bucket index.', de: 'Eine Hashfunktion bildet jeden Schluessel auf einen Behaelterindex ab.' },
      { en: 'Collisions (two keys, same bucket) are handled by chaining (a list per bucket) or open addressing (probe for the next free slot).', de: 'Kollisionen (zwei Schluessel, ein Behaelter) loest man durch Verkettung (Liste pro Behaelter) oder offene Adressierung (naechsten freien Platz suchen).' },
      { en: 'Keep the load factor (items / buckets) low; resize and rehash when it grows.', de: 'Den Fuellgrad (Elemente / Behaelter) niedrig halten; bei Wachstum vergroessern und neu hashen.' },
    ],
    complexity: [
